...
# MAGIC %sql
# MAGIC SELECT * FROM delta.`dbfs:/user/vishal.abnave@borregaard.com/delta/customer-data/`
...
# MAGIC %md
# MAGIC Spark SQL queries (including MERGE) can run directly on a directory of data, for delta use the following syntax: 
# MAGIC
# MAGIC ## MERGE INTO delta.`/path/to/delta_directory` D
# MAGIC ## USING upsert_data_as_Temp_View

# COMMAND ----------

# MAGIC %sql
# MAGIC MERGE INTO delta.`dbfs:/user/vishal.abnave@borregaard.com/delta/customer-data/` CD
# MAGIC USING upsert_data
# MAGIC ON CD.InvoiceNo = upsert_data.InvoiceNo
# MAGIC   AND CD.StockCode = upsert_data.StockCode
# MAGIC WHEN MATCHED THEN
# MAGIC   UPDATE SET *
# MAGIC WHEN NOT MATCHED
# MAGIC   THEN INSERT *
